...
public class QnaServiceTest extends BaseTest {
  ...
  public void 질문_등록() {
    qnaService.create(
        ...
        new Question("국내에서 Ruby on Rails와 Play가 활성화되기 힘든 이유는 뭘까?",
            "Ruby on Rails(이하 RoR)는 2006년 즈음에 정말 뜨겁게 달아올랐다가 금방 가라 앉았다.")
    );
  }
  ...
  public void 내가_작성한_질문_조회() {
    ...
    final Question question1 = createUserAndQuestion(user,
        "국내에서 Ruby on Rails와 Play가 활성화되기 힘든 이유는 뭘까?",
        "Ruby on Rails(이하 RoR)는 2006년 즈음에 정말 뜨겁게 달아올랐다가 금방 가라 앉았다.");
    final Question question2 = createUserAndQuestion(user,
        "국내에서 Ruby on Rails와 Play가 활성화되기 힘든 이유는 뭘까?",
        "Ruby on Rails(이하 RoR)는 2006년 즈음에 정말 뜨겁게 달아올랐다가 금방 가라 앉았다.");
  ...
  public void 내가_작성하지_않은_질문_조회() {
    ...
    final Question question1 = createUserAndQuestion(write,
        "국내에서 Ruby on Rails와 Play가 활성화되기 힘든 이유는 뭘까?",
        "Ruby on Rails(이하 RoR)는 2006년 즈음에 정말 뜨겁게 달아올랐다가 금방 가라 앉았다.");

    when(questionRepository.findById(question1.getId())).thenReturn(Optional.of(question1));

    qnaService.findByUserAndId(
        createUser("ninezero90hy", "ninezero90hy@", "ninezero", "ninezero90hy@gmail.com"),
        question1.getId());
  }
  ...
  public void 질문_수정_성공() {
    ...
    final Question question1 = createUserAndQuestion(user,
        ...
        "Ruby on Rails(이하 RoR)는 2006년 즈음에 정말 뜨겁게 달아올랐다가 금방 가라 앉았다.");
    final Question question2 = createUserAndQuestion(user,
        "국내에서 활성화되기 힘든 이유는 뭘까? Ruby on Rails와 Play가", "ROR는 금방 가라 앉았다.");
  ...
  public void 등록자가_아닌데_질문_수정시() {
    ...
    final Question question1 = createUserAndQuestion(write,
        "국내에서 Ruby on Rails와 Play가 활성화되기 힘든 이유는 뭘까?",
        "Ruby on Rails(이하 RoR)는 2006년 즈음에 정말 뜨겁게 달아올랐다가 금방 가라 앉았다.");
    final Question question2 = createUserAndQuestion(write,
        "국내에서 활성화되기 힘든 이유는 뭘까? Ruby on Rails와 Play가", "ROR는 금방 가라 앉았다.");
    final User user = createUser("ninezero90hy", "ninezero90hy@", "ninezero",
        "ninezero90hy@gmail.com");
  ...
  public void 질문_삭제_성공() throws CannotDeleteException {
    ...
    final User user = createUser("javajigi", "test", "자바지기", "javajigi@slipp.net");
    final Question question = createUserAndQuestion(user,
        "국내에서 Ruby on Rails와 Play가 활성화되기 힘든 이유는 뭘까?",
        "Ruby on Rails(이하 RoR)는 2006년 즈음에 정말 뜨겁게 달아올랐다가 금방 가라 앉았다.");
  ...
  public void 작성자가_아닌데_질문_삭제_시도() throws CannotDeleteException {
    ...
    final User user = createUser("ninezero90hy", "ninezero90hy@", "ninezero",
        "ninezero90hy@gmail.com");
    final Question question = createUserAndQuestion(write,
        "국내에서 Ruby on Rails와 Play가 활성화되기 힘든 이유는 뭘까?",
        "Ruby on Rails(이하 RoR)는 2006년 즈음에 정말 뜨겁게 달아올랐다가 금방 가라 앉았다.");
  ...
  public void 질문에_답변_등록() {
    ...
    final Question question = new Question("국내에서 Ruby on Rails와 Play가 활성화되기 힘든 이유는 뭘까?",
        "Ruby on Rails(이하 RoR)는 2006년 즈음에 정말 뜨겁게 달아올랐다가 금방 가라 앉았다.");
    when(questionRepository.findById(question.getId())).thenReturn(Optional.of(question));
  ...
  public void 질문에_달린_다른_사람의_답변_삭제() {
    ...
    final Answer answer = new Answer(createUser("javajigi", "test", "자바지기", "javajigi@slipp.net"),
        "좋은 책이네요.");
    answer.setId(0);
    when(answerRepository.findById(answer.getId())).thenReturn(Optional.of(answer));
  ...
  public void 질문에_답변_수정() {
    ...
    assertThat(answerRepository.findById(answer.getId()).filter(a -> a.eqContents(updateAnswer))
        .isPresent()).isTrue();
  ...
  public void 질문에_다른_사람의_답변_수정() {
    ...
    final Answer updateAnswer = new Answer(user, "좋은 책이네요!!!");
    qnaService
        .updateAnswer(createUser("javajigi", "test", "자바지기", "javajigi@slipp.net"), updateAnswer);
  }

  private Question createUserAndQuestion(final User user, final String title,
  ...
  private User createUser(final String userId, final String password, final String name,
      final String email) {
